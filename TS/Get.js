...
async function detail(ids) {
    // 猫影视标准传数组 [id]，FongMi/部分壳直接传字符串 id，两者兼容
    const realId = Array.isArray(ids) ? ids[0] : ids;
    const vod = {};
    vod.vod_id = realId;
    try {
        const body = { vod_id: realId };
...
async function play(flag, id, flags) {
    try {
        ...
        let danmaku = '';
        try {
            // 以 & 开头的裸 query：FongMi 系桥会直接拼入 ext（params.do 可取），
            // TVBoxOSC 系桥会整体编码进 url=（proxy 中从 params.url 兜底解析）
            const ext = '&do=appdanmu&vodName=' + encodeURIComponent(vodName) + '&vodIndex=' + encodeURIComponent(nid);
            danmaku = js2Proxy(false, siteType, siteKey || '', ext, {});
        } catch (e) {
            danmaku = '';
        }
...
// 不依赖 QuickJS 运行时可能缺失的 URLSearchParams，自行解析 a=1&b=2
// 各桥对 query 的解码次数不同，做幂等解码：最多两次，遇异常或不再变化即止
...
async function proxy(params) {
    ...
    // 两种桥接形态：
    // 1) FongMi 系：自定义参数裸拼进 ext，直接出现在 params 上
    // 2) TVBoxOSC 系：第 4 参数被整体编码进 url=，需从 params.url 解出
    let doWhat = safeDecode(params.do || '');
    let vodName = safeDecode(params.vodName || '');
    let vodIndex = safeDecode(params.vodIndex || '');
    if (doWhat !== 'appdanmu' && params.url) {
        let raw = safeDecode(params.url);
        if (raw.charAt(0) === '?' || raw.charAt(0) === '&') raw = raw.substring(1);
        const parsed = parseQuery(raw);
        doWhat = parsed.do || doWhat;
        vodName = parsed.vodName || vodName;
        vodIndex = parsed.vodIndex || vodIndex;
    }
    if (doWhat !== 'appdanmu' || !vodName) return emptyResult;
