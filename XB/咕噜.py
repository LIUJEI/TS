...
# ============ 业务解析 ============
def _with_prefix(s):
    """给简介加 DESC_PREFIX 前缀；空/None 返回空串；已有前缀则幂等不重复加。"""
    s = s or ""
    if not s or s.startswith(DESC_PREFIX):
        return s
...
class Spider(_BaseSpider):
    ...
    def detailContent(self, ids):
        ...
        for f, w, v in detail:
            ...
            elif f == 21: vod["vod_content"] = _with_prefix(_safe_str(v))
            ...
            for d in live_list:
                if d["id"] == vid:
                    ...
                    vod = {
                        ...
                        "vod_year": "", "vod_content": _with_prefix(f"{d['name']} 直播频道"), "vod_remarks": "",
